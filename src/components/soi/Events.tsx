  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 md:px-8">
                ...
                <BrutalCard className="flex flex-col overflow-hidden h-full">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <div className="mt-6 flex items-center justify-between border-t-[2px] border-ink pt-4">
                      ...
                      {/* WhatsApp, Registration and PDF Guidelines Buttons */}
                      <div className="flex gap-2.5">
                        ...
                        {e.regLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="grid h-9 w-9 place-items-center border-[2.5px] border-ink bg-[#FFD700] text-ink shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0 cursor-pointer select-none"
        ...
        {/* Inactive Drops Sub Section */}
        {inactiveFilteredEvents.length > 0 && (
          <div className="mt-16 border-t-4 border-dashed border-ink/20 pt-12">
            <h3 className="font-display text-3xl uppercase md:text-5xl mb-8">
              Inactive Drops
            </h3>
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {inactiveFilteredEvents.map((e, i) => (
                <div
                  id={`ps-${e.num}`}
                  key={e.num}
                  className="transition-all duration-500 ease-out scroll-mt-24"
                >
                  <BrutalCard className="flex flex-col overflow-hidden h-full opacity-85 hover:opacity-100 transition-opacity">
                    <div className="relative border-b-[3px] border-ink bg-peach overflow-hidden grayscale-[30%]">
                      <img
                        src={e.image}
                        alt={e.title}
                        width={1024}
                      ...
                      <span
                        className="absolute right-4 top-4 border-[3px] border-ink bg-muted text-muted-foreground px-2 py-1 font-display text-[10px] uppercase shadow-brutal-sm"
                      >
                        {e.date}
                      </span>
                    </div>
                    <div className="flex flex-1 flex-col p-6">
                      <h3 className="font-display text-xl leading-snug md:text-2xl text-foreground/80">
                        {e.title}
                      </h3>
                      ...
                      {/* Done and Dusted Banner */}
                      <div className="mt-3.5 flex items-center gap-1.5">
                        <span className="inline-block border-[2px] border-ink bg-muted text-muted-foreground px-2 py-0.5 font-display text-[9px] font-bold uppercase shadow-brutal-xs">
                          Done and Dusted
                        </span>
                      </div>
                      ...
                      <div className="mt-4 border-t border-dashed border-ink/20 pt-3 flex flex-wrap gap-1.5">
                        {e.domains.map((dom) => (
                          <span
                            key={dom}
                            className="inline-block border-2 border-ink bg-accent/10 text-foreground/60 px-2 py-0.5 font-display text-[9px] font-bold uppercase shadow-brutal-xs"
                          >
                            {dom}
                          </span>
                        ))}
                      ...
                      <div className="mt-6 flex items-center justify-between border-t-[2px] border-ink pt-4">
                        ...
                        <div className="flex gap-2.5">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="grid h-9 w-9 place-items-center border-[2.5px] border-ink bg-[#25D366]/70 text-white shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0"
                            title="Join WhatsApp Group"
                          >
                            <svg
                              viewBox="0 0 24 24"
                          ...
                          {e.regLink && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="grid h-9 w-9 place-items-center border-[2.5px] border-ink bg-[#FFD700]/70 text-ink shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0 cursor-pointer select-none"
                              title="Register for Challenge"
                            >
                              <ClipboardCheck className="h-5 w-5" />
                            </a>
                          ...
                          <button
                            ...
                            className="grid h-9 w-9 place-items-center border-[2.5px] border-ink bg-accent/70 text-ink shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0 cursor-pointer select-none"
                            title="View PS PDF"
                          >
                            <FileText className="h-5 w-5" />
                          </button>

      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {filteredEvents.map((e, i) => {
            const isPast = isDeadlinePassed(e.deadline);
            return (
              <div
                id={`ps-${e.num}`}
                key={e.num}
              ...
              >
                <BrutalCard
                  className={`flex flex-col overflow-hidden h-full transition-opacity ${
                    isPast ? "opacity-85 hover:opacity-100" : ""
                  }`}
                >
                  <div
                    className={`relative border-b-[3px] border-ink bg-peach overflow-hidden ${
                      isPast ? "grayscale-[30%]" : ""
                    }`}
                  >
                    <img
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <p
                      className={`mt-3 flex-1 text-sm leading-relaxed ${
                        isPast ? "text-foreground/60" : "text-foreground/80"
                      }`}
                    >
                      {e.desc}
                    </p>

                    {/* Dynamic deadline / Done and Dusted notification */}
                    {e.deadline && (
                      <div className="mt-3.5 flex items-center gap-1.5">
                        {isPast ? (
                          ...
                          <span className="inline-block border-[2px] border-ink bg-[#ff0000] text-white px-2 py-0.5 font-display text-[9px] font-bold uppercase shadow-brutal-xs animate-pulse">
                            Deadline: {e.deadline}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="mt-4 border-t border-dashed border-ink/20 pt-3 flex flex-wrap gap-1.5">
                      {e.domains.map((dom) => (
                        <span
                          key={dom}
                          className={`inline-block border-2 border-ink px-2 py-0.5 font-display text-[9px] font-bold uppercase shadow-brutal-xs ${
                            isPast
                              ? "bg-accent/10 text-foreground/60"
                              : "bg-accent/20"
                          }`}
                        >
                          {dom}
                    ...
                    <div className="mt-6 flex items-center justify-between border-t-[2px] border-ink pt-4">
                      ...
                      <div className="flex gap-2.5">
                        ...
                        <a
                          ...
                          className={`grid h-9 w-9 place-items-center border-[2.5px] border-ink text-white shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0 ${
                            isPast ? "bg-[#25D366]/70" : "bg-[#25D366]"
                          }`}
                        ...
                        {e.regLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className={`grid h-9 w-9 place-items-center border-[2.5px] border-ink text-ink shadow-brutal-sm transition-transform hover:translate-x-[1.5px] hover:translate-y-[1.5px] hover:shadow-none active:translate-x-0 active:translate-y-0 cursor-pointer select-none ${
                              isPast ? "bg-[#FFD700]/70" : "bg-[#FFD700]"
                            }`}
